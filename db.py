...
def parser(url):
	...
	apiSc = requests.post(apiUrl,data={'path':mainPath,'host':'moeni.org'},headers={'content-type': 'application/x-www-form-urlencoded','referer':url,'user-agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.77 Safari/537.36 Edg/91.0.864.37'})
		...
		for opt in optg:
			...
			for ee in opt.find_all('option'):
				...
				if ee.get('value') != None and ee.get('c') != None:
					...
					link2 = ee['c']
					url = f'https://player.moeni.org/play.php?c={link2}&v={link1}'
					urll.append({'series':series,'episode':episode,'url':url})
				#else: 
					#print(ee)
...
def getMagnet(dict,referer):
	#print("[MoeniMagnet] getMagnet_S\r")
	url = dict['url']
...
for i in range(start,len(stmap)-1):
	url = str(stmap[i])
	# url = 'https://moeni.org/%ec%95%85%eb%a7%88%ec%97%90-%ec%9e%85%eb%ac%b8%ed%96%88%ec%8a%b5%eb%8b%88%eb%8b%a4-%ec%9d%b4%eb%a3%a8%eb%a7%88-%ea%b5%b0/'
...
#		print(f"[MoeniMagnet] [{title}] {i['episode']}")
	...
	try:
		...
		for eplist in url_list[0]:
			...
			magnet = ml[0]
			result.append({'series':eplist['series'],'episode':eplist['episode'],'magnet':magnet,'webSeed':ml[2],'sub':ml[1],'referer':referer})
		#print(result)
		c.execute(f"INSERT INTO anime VALUES({i+1},'{onair}','{title}','{url}')")
		for res in result:
			c.execute(f"INSERT INTO magnet VALUES({i+1}, '{res['series']}','{res['episode']}','{res['magnet']}','{res['webSeed']}','{res['sub']}','{referer}')")
